...
namespace logAxe
{
    public partial class frmLineData : Form
    {
        ...
        public void SetCurrentLine(int currentLine, int totalLine, LogLine lineInfo)
        {
            ...
            var timeStamp = $"{lineInfo.TimeStamp:yyyy-MM-dd hh:mm:ss.fff}";
            ...
            textBox1.Text =
                    ...
                    //$"MsgId: {lineInfo.MsgId}, StackId: {lineInfo.MsgId}, ThreadId: {lineInfo.ThreadNo}, ProcId: {lineInfo.ProcessId}, Category: {lineInfo.Category} {Environment.NewLine}" +
                    //$"MsgId: {lineInfo.MsgId}, StackId: {lineInfo.MsgId}, ThreadId: {lineInfo.ThreadNo}, ProcId: {lineInfo.ProcessId}, Category: {lineInfo.Category} {Environment.NewLine}" +
        ...
        private void handle_enter_key_for_time(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Return)
            {
                try
                {
                    DateTime.ParseExact(txtTimeTravel.Text, "yyyy-MM-dd hh:mm:ss.fff", CultureInfo.InvariantCulture);                   
                }
                catch
                {
                    txtTimeTravel.Text = _lasttimeStamp;
